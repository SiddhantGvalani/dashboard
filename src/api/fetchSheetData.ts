...
async function getAccessToken(serviceAccount: Record<string, string>): Promise<string> {
  ...
  if (!tokenData.access_token) {
    throw new Error(`Google OAuth failed: ${tokenData.error ?? JSON.stringify(tokenData)}`);
  }
...
export default createEndpoint({
  description: 'Fetches rows from a Google Sheet using a Service Account. Supports Reference Start Row. Strips rows with no ETD.',
